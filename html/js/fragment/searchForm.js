define(['../data/getData','../moduls/Tool'],function(getData,Tool){
	var list =  [ //表单
		...
		},
		{
			...
			selectName : ['fragmentClassifyId'],
			name : '碎片分类',
			type : 'select',
			select : [
				[
					{name:'请选择碎片分类',title:'fragmentClassifyId'}
				]
			]
		},
		{
	...
	function getList(callback){
		getData.fragment.listClassify({//系列专题是。topicColumn
					...
					if($.type(obj)=='array'){
						$.each(obj,function(){
							setData({
								...
								data : _data , //数据
								changeName : 'classifyName' //需要显示字段的 name 名称
							})
						})
					}else{
						setData({
